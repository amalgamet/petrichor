...
describe('pairForecastDays', () => {
  ...
  it('handles a full week of periods', () => {
    ...
    expect(result[0].highTemp).toBe(40);
    expect(result[0].lowTemp).toBe(25);
    expect(result[6].highTemp).toBe(46);
    expect(result[6].lowTemp).toBe(31);
  });
});
...
describe('calcTempRange', () => {
  it('returns min and max across all days', () => {
    const days = [
      { dayName: 'Mon', date: '2026-02-23', highTemp: 45, lowTemp: 30, shortForecast: '', precipChance: null },
      { dayName: 'Tue', date: '2026-02-24', highTemp: 50, lowTemp: 25, shortForecast: '', precipChance: null },
      { dayName: 'Wed', date: '2026-02-25', highTemp: 42, lowTemp: 35, shortForecast: '', precipChance: null },
    ];

    expect(calcTempRange(days)).toEqual({ weekMin: 25, weekMax: 50 });
  });

  it('handles days with null highTemp', () => {
    const days = [
      { dayName: 'Tonight', date: '2026-02-22', highTemp: null, lowTemp: 28, shortForecast: '', precipChance: null },
      { dayName: 'Mon', date: '2026-02-23', highTemp: 45, lowTemp: 30, shortForecast: '', precipChance: null },
    ];
  ...
  it('handles days with null lowTemp', () => {
    const days = [
      { dayName: 'Sat', date: '2026-03-01', highTemp: 50, lowTemp: null, shortForecast: '', precipChance: null },
    ];
  ...
  it('handles all identical temperatures', () => {
    const days = [
      { dayName: 'Mon', date: '2026-02-23', highTemp: 40, lowTemp: 40, shortForecast: '', precipChance: null },
    ];
...
describe('tempRangeBar', () => {
  it('calculates left offset and width as percentages', () => {
    const day = { dayName: 'Mon', date: '2026-02-23', highTemp: 40, lowTemp: 30, shortForecast: '', precipChance: null };
  ...
  it('returns full width when weekMin equals weekMax', () => {
    const day = { dayName: 'Mon', date: '2026-02-23', highTemp: 40, lowTemp: 40, shortForecast: '', precipChance: null };
  ...
  it('handles null lowTemp by using highTemp', () => {
    const day = { dayName: 'Sat', date: '2026-03-01', highTemp: 50, lowTemp: null, shortForecast: '', precipChance: null };

    const result = tempRangeBar(day, 20, 60);

    // lowTemp falls back to highTemp (50), so: left=(50-20)/40=75%, width=(50-50)/40=0%
    expect(result.leftPct).toBe(75);
    expect(result.widthPct).toBe(0);
  });

  it('handles null highTemp by using lowTemp', () => {
    const day = { dayName: 'Tonight', date: '2026-02-22', highTemp: null, lowTemp: 28, shortForecast: '', precipChance: null };

    const result = tempRangeBar(day, 20, 60);

    // highTemp falls back to lowTemp (28), so: left=(28-20)/40=20%, width=(28-28)/40=0%
    expect(result.leftPct).toBe(20);
    expect(result.widthPct).toBe(0);
  });

  it('spans the full range for the most extreme day', () => {
    const day = { dayName: 'Mon', date: '2026-02-23', highTemp: 60, lowTemp: 20, shortForecast: '', precipChance: null };
